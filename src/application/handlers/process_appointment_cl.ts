  ...
  for (const record of event.Records) {
    try {
      ...
      if (!message || !message.data || !message.data.data) {
        console.error( "Estructura de mensaje inválida:", JSON.stringify(message));
        throw new Error("Estructura de mensaje inválida");
      }
      const appointmentData = message.data.data;

      // implementa la lógica específica para CL
      await processAppointmentCL(appointmentData, dbPool);

      // si el procesamiento fue exitoso, eliminamos el mensaje de la cola
    } catch (error) {
      console.error("Error procesando mensaje para CL:", error);
...
async function processAppointmentCL( appointment: any, pool: mysql.Pool): Promise<void> {
  try {
    ...
    if (!appointment.appointmentId || !appointment.insuredId) {
      console.error("Faltan datos obligatorios para la cita");
      throw new Error("Faltan datos obligatorios para la cita");
    }

    // formatea fecha si es necesario
    let dateSchedule;
    try {
    ...
    // envia confirmación de cita al EventBridge
    sendAppointmentConfirmation(appointment, "CL")
      .then(() =>
        console.log( `Confirmación enviada en segundo plano para ${appointment.appointmentId}` )
      )
      .catch((err) =>
        console.error( `Error enviando confirmación para ${appointment.appointmentId}:`, err )
      );
  } catch (error) {
    console.error("Error en el procesamiento para CL :", error);
    throw error;
  }
}
